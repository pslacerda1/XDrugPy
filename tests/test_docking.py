...
def test_vina_engine():
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        assert (
            "Adding receptor hydrogens:"
            in eng1.manager.logEvent.emit.call_args_list[0][0][0]
        )
        assert (
            "Preparing receptor." in eng1.manager.logEvent.emit.call_args_list[1][0][0]
        )
        assert (
            "Stored receptor at:" in eng1.manager.logEvent.emit.call_args_list[3][0][0]
        )
        ...
        assert (
            "Scrubbing ligands." in eng1.manager.logEvent.emit.call_args_list[4][0][0]
        )
        scrub_log = eng1.manager.logEvent.emit.call_args_list[5][0][0]
        assert (
            "nr conformers:  5" in scrub_log or "nr conformers:  4 " in scrub_log
        )  # depending on scrub BUG
        ...
        assert (
            ...
            in eng1.manager.logEvent.emit.call_args_list[6][0][0]
        )
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        eng2.run_docking()
        assert (
            "--cpu 1 --seed 42 --size_x 11.11 --size_y 10.48 --size_z 12.50 --center_x 16.55 --center_y -14.26 --center_z 8.36"
            in eng2.manager.logEvent.emit.call_args_list[2][0][0]
        )
        assert (
            "<b>Results:</b> 5" in eng2.manager.logEvent.emit.call_args_list[4][0][0]
            or "<b>Results:</b> 4" in eng2.manager.logEvent.emit.call_args_list[4][0][0]
        )  # depending on scrub BUG
        ...
        assert (
            "Continuating docking at:"
            in eng3.manager.logEvent.emit.call_args_list[1][0][0]
        )
        assert (
            "<b>Results:</b> 5" in eng3.manager.logEvent.emit.call_args_list[4][0][0]
            or "<b>Results:</b> 4" in eng3.manager.logEvent.emit.call_args_list[4][0][0]
        )  # depending on scrub BUG
