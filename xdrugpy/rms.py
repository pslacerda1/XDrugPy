...
from .utils import ONE_LETTER, declare_command, mpl_axis, get_mapping
...
def rmsf(
    ...
    """
    DESCRIPTION
        Calculate the RMSF of muliple related structures. First it does
        multiple sequence/structure alignment with the cealign function in
        order to get equivalent atoms, so it can be realized between
...
#     mappings = get_mapping(frames, ref_site, site_margin)
...
#             if i1 >= i2:
#                 continue
#             pm.cealign(f"{f1} & polymer", f"{f2} & polymer", object=f"{f1}_{f2}")
#             resis, coordinates = get_residues(f"{f1}_{f2}")
#             for resi, coords in zip(resis, coordinates):
#                 if resi not in X:
#                     X[resi] = np.empty((0,3))
#                 X[resi] = np.vstack([X[resi], coords])
#         resis, coordinates = get_residues(f1)
#         for resi, coords in zip(resis, coordinates):
#             if resi not in X:
#                 X[resi] = np.empty((0,3))
#             X[resi] = np.vstack([X[resi], coords])
